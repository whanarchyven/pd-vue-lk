  ...
  <div>
    ...
    >
      <template v-slot:body.prepend>
        <filters :columns="columns" :dates="dates" @inputKeyup="inputKeyup" @dateFilterChanged="saveDate" @dateFilterCleared="clearDate"></filters>
      </template>
      ...
      <template v-slot:item="{ item }">
        <tr>
          <td v-for="column in columns" class="text-start">
            <router-link class="data-table-link" :to="{ name: 'Book', params: { id: item.id } }">
              <span v-if="column.class && column.class.dynamic" :class="column.class.name + item[column.name]">
                {{ column.naming ? column.naming[item[column.name]] : item[column.name] }}
              </span>
